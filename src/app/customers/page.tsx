            ...
            <div className="flex h-full min-h-[700px] flex-col justify-between bg-[#181114] p-4">
              <div className="flex flex-col gap-4">
                ...
                <nav className="flex flex-col gap-2">
                  ...
                  <SidebarLink icon={UsersIcon} label="Customers" href="/customers" active />
...
function CustomersTable() {
  ...
  React.useEffect(() => {
    const fetchCustomers = async () => {
      ...
      if (debouncedSearch.trim()) {
        query = query.or(`phone.ilike.%${debouncedSearch}%,body_shape.ilike.%${debouncedSearch}%,skin_tone.ilike.%${debouncedSearch}%,gender.ilike.%${debouncedSearch}%`);
      }
      // Fetch total count
  ...
  if (loading) {
    return <div className="text-white p-4">Loading customers...</div>;
  }
  if (error) {
      ...
      {pendingDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
          <div className="bg-gradient-to-br from-[#22161b] to-[#271b20] p-6 rounded-xl shadow-2xl w-full max-w-sm flex flex-col gap-4 border border-[#39282e] animate-fadeIn">
            <h2 className="text-white text-lg font-extrabold mb-1 tracking-tight text-center pb-1 border-b border-[#39282e]">Delete Customer</h2>
            <p className="text-[#ba9ca7] text-center">Are you sure you want to delete this customer?</p>
            <div className="flex flex-col items-center gap-1">
              <span className="text-white font-semibold">{pendingDelete}</span>
            </div>
      ...
      {deleteSuccess && (
        <div className="fixed inset-0 z-60 flex items-center justify-center bg-black bg-opacity-40 animate-fadeIn">
          <div className="bg-gradient-to-br from-[#22161b] to-[#271b20] border border-[#39282e] rounded-2xl shadow-2xl px-8 py-8 flex flex-col items-center gap-3 animate-fadeInUp relative min-w-[280px]">
            ...
            <div className="text-white text-lg font-bold text-center">Customer Deleted!</div>
            <div className="text-[#b16cea] text-sm text-center font-medium">The customer was deleted successfully.</div>
          </div>
        </div>
      )}
      ...
      <table className="flex-1 min-w-full">
        <thead>
          <tr className="bg-[#271b20]">
            <th className="px-4 py-3 text-left text-white text-sm font-medium leading-normal">ID</th>
            <th className="px-4 py-3 text-left text-white text-sm font-medium leading-normal">Phone</th>
            <th className="px-4 py-3 text-left text-white text-sm font-medium leading-normal">Height</th>
            <th className="px-4 py-3 text-left text-white text-sm font-medium leading-normal">Weight</th>
            <th className="px-4 py-3 text-left text-white text-sm font-medium leading-normal">Body Shape</th>
            <th className="px-4 py-3 text-left text-white text-sm font-medium leading-normal">Skin Tone</th>
            <th className="px-4 py-3 text-left text-white text-sm font-medium leading-normal">Gender</th>
            <th className="px-4 py-3 text-center text-white text-sm font-medium leading-normal">Actions</th>
          </tr>
        </thead>
        <tbody>
          {customers.map((customer) => (
            <tr key={customer.id} className="border-t border-t-[#543b44]">
              <td className="h-[56px] px-4 py-2 text-white text-sm font-normal leading-normal">{customer.id}</td>
              <td className="h-[56px] px-4 py-2 text-[#ba9ca7] text-sm font-normal leading-normal">{customer.phone}</td>
              <td className="h-[56px] px-4 py-2 text-[#ba9ca7] text-sm font-normal leading-normal">{customer.height}</td>
              <td className="h-[56px] px-4 py-2 text-[#ba9ca7] text-sm font-normal leading-normal">{customer.weight}</td>
              <td className="h-[56px] px-4 py-2 text-[#ba9ca7] text-sm font-normal leading-normal">{customer.body_shape}</td>
              <td className="h-[56px] px-4 py-2 text-[#ba9ca7] text-sm font-normal leading-normal">{customer.skin_tone}</td>
              <td className="h-[56px] px-4 py-2 text-[#ba9ca7] text-sm font-normal leading-normal">{customer.gender}</td>
