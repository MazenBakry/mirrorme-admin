  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      const { data: catData } = await supabase
        ...
        .select('category');
      if (catData) {
        const uniqueCategories = new Set((catData as Product[]).map((p) => p.category));
      ...
      if (priceData && priceData.length > 0) {
        const avg = (priceData as Product[]).reduce((sum, p) => sum + (p.price || 0), 0) / priceData.length;
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      ...
      const { data, error } = await supabase
        ...
        .select('category');
      if (!error && data) {
        const unique = Array.from(new Set((data as { category: string }[]).map((p) => p.category).filter(Boolean)));
      ...
      <div className="layout-container flex h-full grow flex-col">
        <div className="gap-1 px-6 flex flex-1 justify-center py-5">
            ...
            <div className="flex h-full min-h-[700px] flex-col justify-between bg-[#181114] p-4">
              <div className="flex flex-col gap-4">
                ...
                <nav className="flex flex-col gap-2">
                  ...
                  <SidebarLink icon={PackageIcon} label="Inventory" href="/Inventory" active />
          ...
          <main className="layout-content-container flex flex-col max-w-[960px] flex-1">
            ...
            {showAddModal && <AddProductModal onClose={() => setShowAddModal(false)} categories={categories} setCategories={setCategories} catLoading={catLoading} />}
            <div className="flex flex-wrap gap-4 p-4">
              <StatCard label="Total Products" value={totalProducts !== null ? totalProducts.toString() : '...'} />
              <StatCard label="Total Categories" value={totalCategories !== null ? totalCategories.toString() : '...'} />
              <StatCard label="Average Price" value={averagePrice !== null ? `$${averagePrice}` : '...'} />
              {Object.keys(genderStats).map((gender) => (
                <StatCard key={gender} label={`${gender.charAt(0).toUpperCase() + gender.slice(1)} Products`} value={genderStats[gender].toString()} />
              ))}
            </div>
            ...
            <div className="px-4 py-3">
              <div className="flex overflow-x-auto rounded-xl border border-[#543b44] bg-[#181114]">
                <ProductsTable categories={categories} setCategories={setCategories} catLoading={catLoading} />
              </div>
            </div>
          </main>
        </div>
...
function ProductsTable({ categories, setCategories, catLoading }: { categories: string[], setCategories: (c: string[]) => void, catLoading: boolean }) {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      if (debouncedSearch.trim()) {
        query = query.or(`name.ilike.%${debouncedSearch}%,category.ilike.%${debouncedSearch}%,gender.ilike.%${debouncedSearch}%`);
      }
  ...
  if (loading) {
    return <div className="text-white p-4">Loading products...</div>;
  }
  if (error) {
      ...
      {pendingDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
          <div className="bg-gradient-to-br from-[#22161b] to-[#271b20] p-6 rounded-xl shadow-2xl w-full max-w-sm flex flex-col gap-4 border border-[#39282e] animate-fadeIn">
            <h2 className="text-white text-lg font-extrabold mb-1 tracking-tight text-center pb-1 border-b border-[#39282e]">Delete Product</h2>
            <p className="text-[#ba9ca7] text-center">Are you sure you want to delete this product?</p>
            <div className="flex flex-col items-center gap-2">
              <span className="text-white font-semibold">ID: {pendingDelete.id}</span>
              <img src={pendingDelete.image_url} alt={pendingDelete.name} className="w-24 h-24 object-cover rounded border border-[#543b44]" />
            </div>
            <div className="flex gap-4 justify-center mt-2">
              ...
              <button
                className="px-4 py-2 rounded-lg bg-gradient-to-r from-red-700 to-red-500 text-white font-semibold hover:from-red-800 hover:to-red-600 transition shadow"
        ...
        <div className="fixed inset-0 z-60 flex items-center justify-center bg-black bg-opacity-40 animate-fadeIn">
          <div className="bg-gradient-to-br from-[#22161b] to-[#271b20] border border-[#39282e] rounded-2xl shadow-2xl px-8 py-8 flex flex-col items-center gap-3 animate-fadeInUp relative min-w-[280px]">
            ...
            <div className="flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-br from-[#b16cea] to-[#ff5e69] mb-2 shadow-lg">
              <svg width="40" height="40" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="12" r="12" fill="none"/><path d="M7 13l3 3 7-7" stroke="#fff" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
            </div>
            <div className="text-white text-lg font-bold text-center">Product Deleted!</div>
            <div className="text-[#b16cea] text-sm text-center font-medium">The product was deleted successfully.</div>
          </div>
        </div>
      )}
      ...
      <table className="flex-1 min-w-full">
        <thead>
          <tr className="bg-[#271b20]">
            <th className="px-4 py-3 text-left text-white w-40 text-sm font-medium leading-normal">
              Product ID
            </th>
            <th className="px-4 py-3 text-left text-white w-32 text-sm font-medium leading-normal">
              Image
            </th>
            <th className="px-4 py-3 text-left text-white w-64 text-sm font-medium leading-normal">
              Name
            </th>
            <th className="px-4 py-3 text-left text-white w-40 text-sm font-medium leading-normal">
              Category
            ...
            <tr key={product.id} className="border-t border-t-[#543b44]">
              ...
              <td className="h-[56px] px-4 py-2 w-32">
                <div className="flex gap-2 items-center justify-center h-full">
                  ...
                  <button
                    className="px-3 py-1 rounded-lg bg-gradient-to-r from-red-700 to-red-500 text-white font-semibold text-xs hover:from-red-800 hover:to-red-600 transition shadow"
        ...
        <nav className="flex gap-2 bg-[#22161b] rounded-xl px-4 py-2 shadow-lg">
          ...
          {pageNumbers.map((num, idx) =>
            typeof num === 'number' ? (
              ...
              <span key={`ellipsis-${idx}`} className="px-2 py-1 text-[#ba9ca7] select-none">...</span>
            )
          )}
          <button
            className="px-3 py-1 rounded-lg text-white bg-[#39282e] hover:bg-[#543b44] transition disabled:opacity-40"
...
function AddProductModal({ onClose, categories, setCategories, catLoading }: { onClose: () => void, categories: string[], setCategories: (c: string[]) => void, catLoading: boolean }) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    if (e.target.name === 'image_file' && e.target instanceof HTMLInputElement && e.target.files) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const categoryToUse = showNewCategoryInput && newCategory.trim() ? newCategory.trim() : form.category;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
      <form
        ...
        style={{ minWidth: 260 }}
      >
        <h2 className="text-white text-lg font-extrabold mb-1 tracking-tight text-center pb-1 border-b border-[#39282e]">Add Product</h2>
        {/* Name */}
        <div className="relative">
          <input
            ...
            required
          />
          <label htmlFor="add-name" className="absolute left-3 top-1 text-[#b16cea] text-xs font-semibold pointer-events-none transition-all peer-focus:top-0.5 peer-focus:text-xs peer-focus:text-[#b16cea] peer-placeholder-shown:top-5 peer-placeholder-shown:text-sm peer-placeholder-shown:text-[#ba9ca7]">Name</label>
        </div>
        {/* Image File */}
        <div className="relative flex flex-col gap-2">
          <label htmlFor="add-image-file" className="text-[#b16cea] text-xs font-semibold">Product Image</label>
          <input
            name="image_file"
            id="add-image-file"
            type="file"
          ...
          />
          {imagePreview && (
            <img src={imagePreview} alt="Preview" className="w-24 h-24 object-cover rounded border border-[#543b44] mt-2 mx-auto" />
          )}
        </div>
        {/* Price */}
        <div className="relative">
          <input
            ...
            required
          />
          <label htmlFor="add-price" className="absolute left-3 top-1 text-[#b16cea] text-xs font-semibold pointer-events-none transition-all peer-focus:top-0.5 peer-focus:text-xs peer-focus:text-[#b16cea] peer-placeholder-shown:top-5 peer-placeholder-shown:text-sm peer-placeholder-shown:text-[#ba9ca7]">Price</label>
        </div>
        ...
        <div className="relative">
          ...
          <label htmlFor="add-category" className="absolute left-3 top-1 text-[#b16cea] text-xs font-semibold pointer-events-none transition-all peer-focus:top-0.5 peer-focus:text-xs peer-focus:text-[#b16cea] peer-placeholder-shown:top-5 peer-placeholder-shown:text-sm peer-placeholder-shown:text-[#ba9ca7]">Category</label>
          {/* Custom arrow */}
        ...
        {error && <div className="text-red-400 text-center bg-[#39282e] rounded-lg py-1 px-2 font-semibold shadow mt-1 text-sm">{error}</div>}
        {success && (
          <div className="fixed inset-0 z-60 flex items-center justify-center bg-black bg-opacity-40 animate-fadeIn">
            <div className="bg-gradient-to-br from-[#22161b] to-[#271b20] border border-[#39282e] rounded-2xl shadow-2xl px-8 py-8 flex flex-col items-center gap-3 animate-fadeInUp relative min-w-[280px]">
              <button
              ...
              <div className="text-white text-lg font-bold text-center">Product Added!</div>
              <div className="text-[#b16cea] text-sm text-center font-medium">Your product was added successfully.</div>
            </div>
          </div>
        )}
        <div className="flex gap-2 mt-2">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 bg-gradient-to-r from-gray-600 to-gray-800 text-white py-2 rounded-lg font-bold text-base hover:from-gray-700 hover:to-gray-900 transition shadow border-none"
            disabled={loading}
          >
            Cancel
          </button>
...
function EditProductModal({ product, onClose, onSave, categories, setCategories, catLoading }: { product: Product, onClose: () => void, onSave: (p: Product) => void, categories: string[], setCategories: (c: string[]) => void, catLoading: boolean }) {
  ...
  const [form, setForm] = useState({ ...product, gender: normalizeGender(product.gender) });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSave({ ...form, price: parseFloat(String(form.price)), image_url: imageUrl, category: categoryToUse });
      if (showNewCategoryInput && newCategory.trim() && !categories.includes(newCategory.trim())) {
        setCategories([newCategory.trim(), ...categories]);
      }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
      <form
        ...
        style={{ minWidth: 260 }}
      >
        <h2 className="text-white text-lg font-extrabold mb-1 tracking-tight text-center pb-1 border-b border-[#39282e]">Edit Product</h2>
        {/* Name */}
        <div className="relative">
          <input
            name="name"
            ...
            required
          />
          <label htmlFor="edit-name" className="absolute left-3 top-1 text-[#b16cea] text-xs font-semibold pointer-events-none transition-all peer-focus:top-0.5 peer-focus:text-xs peer-focus:text-[#b16cea] peer-placeholder-shown:top-5 peer-placeholder-shown:text-sm peer-placeholder-shown:text-[#ba9ca7]">Name</label>
        </div>
        {/* Image File */}
        <div className="relative flex flex-col gap-2">
          <label htmlFor="edit-image-file" className="text-[#b16cea] text-xs font-semibold">Product Image</label>
          <input
            name="image_file"
            id="edit-image-file"
            type="file"
        ...
        <div className="relative">
          <input
            ...
            required
          />
          <label htmlFor="edit-price" className="absolute left-3 top-1 text-[#b16cea] text-xs font-semibold pointer-events-none transition-all peer-focus:top-0.5 peer-focus:text-xs peer-focus:text-[#b16cea] peer-placeholder-shown:top-5 peer-placeholder-shown:text-sm peer-placeholder-shown:text-[#ba9ca7]">Price</label>
        </div>
        {/* Category select box - modern UI */}
        <div className="relative">
          <select
          ...
          <label htmlFor="edit-category" className="absolute left-3 top-1 text-[#b16cea] text-xs font-semibold pointer-events-none transition-all peer-focus:top-0.5 peer-focus:text-xs peer-focus:text-[#b16cea] peer-placeholder-shown:top-5 peer-placeholder-shown:text-sm peer-placeholder-shown:text-[#ba9ca7]">Category</label>
          {/* Custom arrow */}
          <div className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 text-[#b16cea]">
            <svg width="16" height="16" fill="none" viewBox="0 0 24 24"><path d="M7 10l5 5 5-5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
          </div>
        ...
        {success && (
          <div className="fixed inset-0 z-60 flex items-center justify-center bg-black bg-opacity-40 animate-fadeIn">
            <div className="bg-gradient-to-br from-[#22161b] to-[#271b20] border border-[#39282e] rounded-2xl shadow-2xl px-8 py-8 flex flex-col items-center gap-3 animate-fadeInUp relative min-w-[280px]">
              ...
              <div className="text-white text-lg font-bold text-center">Product Edited!</div>
              <div className="text-[#b16cea] text-sm text-center font-medium">Your changes were saved successfully.</div>
            </div>
          </div>
        )}
        <div className="flex gap-2 mt-2">
